...
uint256 hashGenesisBlock("0x000005c7eba5af0ed65a95abbc1bc642b1dda80b0cf542fe512faf1da8964b88"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TMCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TMCoin Signed Message:\n";
...
{
    // TMCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)
		nSubsidy = 2900000 * COIN;
	else
		nSubsidy = 0.06342 * COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60 ; // TMCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // TMCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TMCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, tmcoin - DarkGravity v2, written by Evan Duffield - evan@tmcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TMCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // TMCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "bitcoin price coimes to 277$ on bitfinex 2015/3/8";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xa60293e06b344032302eac7219e2930526948c8ec202677c78439585d0f7a91b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TMCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TMCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TMCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TMCoinMiner(CWallet *pwallet)
{
    printf("TMCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tmcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TMCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TMCoinMiner, pwallet));
}
